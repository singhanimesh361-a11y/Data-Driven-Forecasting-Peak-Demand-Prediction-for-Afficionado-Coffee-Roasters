...
class ModelRegistry:
    ...
    def __init__(self, tracking_uri: str = 'sqlite:///mlruns.db', experiment_name: str = 'adip-forecasting'):
        ...
        logger.info(f"Initialized ModelRegistry with tracking URI: {self.tracking_uri}, experiment: {self.experiment_name}")
    ...
    def log_training_run(self,
                         ...
                         model_obj: Any,
                         artifact_paths: Optional[List[str]] = None) -> str:
    ...
    def load_best_model(self, model_name: str, store_id: int, metric: str = 'mape', lower_is_better: bool = True) -> Any:
        ...
        for model_name in models_to_train:
            try:
                ...
                if model_name == 'Naive':
                    ...
                    model = LSTMForecaster(n_optuna_trials=3, epochs=10).fit(store_train, feature_cols=fp.get_feature_names())
                elif model_name == 'Ensemble':
                    if 'Prophet' in trained_models and 'XGBoost' in trained_models:
                        model = EnsembleForecaster(prophet=trained_models['Prophet'], xgb=trained_models['XGBoost'], lstm=trained_models.get('LSTM'))
